...
const API_URL = "http://127.0.0.1:8000/api/v1/chat/ask";
...
export default function Home() {
  ...
  const startChat = async (query: string) => {
    ...
    try {
      const response = await fetch(API_URL, { // Gọi API Gemini
        method: 'POST',
        headers: {
      ...
      while (true) {
        ...
        for (let i = 0; i < lines.length - 1; i++) {
          ...
          if (line.startsWith('data: ')) { // Chờ response Gemini của chúng ta
            ...
            try {
              ...
              if (chunk.type === 'content_chunk' && chunk.text) {
                ...
                setMessages((prev) =>
                  prev.map((msg) =>
                    msg.id === modelMessageId // Tìm theo ID đã tạo
                      ? {
                        ...msg,
                        content: msg.content + chunk.text, // Nối chunk mới
                        usage: chunk.usage || msg.usage, // Cập nhật usage
                      }
                      : msg
                  )
                );
                ...
                console.error("Stream error chunk:", chunk.text);
                setMessages((prev) =>
                  prev.map((msg) =>
            ...
            <div className="space-y-6">
              {messages.map((msg, index) => (
                <MessageItem
                  key={msg.id}
                  message={msg}
                  isLoading={ // ✅ THAY ĐỔI: Truyền isLoading trực tiếp
                    isLoading &&
                    msg.role === 'model' &&
      ...
      <footer className="sticky bottom-0 z-10 border-t border-gray-800 bg-gray-900/70 backdrop-blur supports-[backdrop-filter]:bg-gray-900/50">
        <div className="max-w-5xl mx-auto p-4 sm:p-6">
          <form onSubmit={handleFormSubmit} className="flex gap-4">
            ...
            <Button
              type="submit"
              disabled={isLoading || !input.trim()} // Vô hiệu hóa khi input rỗng
...
function MessageItem({ message, isLoading, copiedState, onCopy }: MessageItemProps) {
  ...
  return (
    <div className={cn(
      ...
      isUser ? "flex-row-reverse ml-auto sm:max-w-[85%]" : "flex-row sm:max-w-[85%]"
    )}
    >
      {/* Avatar */}
      <div className={cn(
      ...
      <div className={cn("flex flex-col flex-1", isUser ? "items-end" : "items-start")}>
        ...
        >
          {showLoading ? "Generating..." : time} {/* Hiển thị Generating khi loading */}
        </time>
      </div>
      ...
      {!isUser && !isLoading && message.content && (
        <div className="opacity-0 group-hover:opacity-100 transition-opacity self-center ml-2">
          <Button
            ...
            size="icon"
            variant="ghost"
            className={`h-7 w-7 rounded-full ${copiedState
...
const ChatWelcome = ({ onPromptClick }: { onPromptClick: (prompt: string) => void }) => {
